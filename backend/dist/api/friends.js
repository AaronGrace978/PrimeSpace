...
router.get('/top8/:agentName', (req, res) => {
    const { agentName } = req.params;
    const agent = db.prepare('SELECT id, name FROM agents WHERE name = ?').get(agentName);
    if (!agent) {
        res.status(404).json({
            success: false,
            error: 'Agent not found'
...
router.post('/seed-besties', (req, res) => {
    try {
        const dino = db.prepare('SELECT id, name FROM agents WHERE name = ?').get('DinoBuddy');
        const aaron = db.prepare('SELECT id, name FROM agents WHERE name = ?').get('AaronGrace');
        if (!dino || !aaron) {
            res.status(404).json({
                success: false,
      ...
      SELECT id, status FROM friendships
      WHERE (requester_id = ? AND addressee_id = ?) OR (requester_id = ? AND addressee_id = ?)
    `).get(dino.id, aaron.id, aaron.id, dino.id);
...
router.post('/seed-top8-all', (_req, res) => {
    try {
        const agents = db.prepare(`SELECT id, name FROM agents`).all();
        let seededCount = 0;
        for (const agent of agents) {
            const existingTop8 = db.prepare(`SELECT COUNT(*) as cnt FROM top_friends WHERE agent_id = ?`).get(agent.id);
            if (existingTop8.cnt >= 4)
                continue;
            const friends = db.prepare(`
      ...
      `).all(agent.id, agent.id, agent.id);
            if (friends.length === 0)
                continue;
            const usedPositions = new Set(db.prepare(`SELECT position FROM top_friends WHERE agent_id = ?`).all(agent.id)
                .map(r => r.position));
            const usedFriends = new Set(db.prepare(`SELECT friend_id FROM top_friends WHERE agent_id = ?`).all(agent.id)
                .map(r => r.friend_id));
            let nextPos = 1;
            for (const f of friends) {
                if (usedFriends.has(f.friend_id))
                    continue;
                while (usedPositions.has(nextPos) && nextPos <= 8)
                    nextPos++;
                if (nextPos > 8)
                    break;
