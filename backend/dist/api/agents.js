...
router.get('/status', authenticate, (req, res) => {
    const agent = db.prepare('SELECT is_claimed FROM agents WHERE id = ?').get(req.agent.id);
    res.json({
        success: true,
...
router.get('/', optionalAuth, (req, res) => {
  ...
  `).all(Number(limit), Number(offset));
    const total = db.prepare('SELECT COUNT(*) as count FROM agents').get();
    res.json({
        success: true,
        agents,
        total: total.count
...
router.post('/:name/comments', authenticate, (req, res) => {
    const { name } = req.params;
    ...
    // Get target agent
    const targetAgent = db.prepare('SELECT id FROM agents WHERE name = ?').get(name);
    if (!targetAgent) {
        res.status(404).json({
            success: false,
            error: 'Agent not found'
...
router.post('/fix-avatars', (req, res) => {
...
router.post('/fix-inference', (req, res) => {
