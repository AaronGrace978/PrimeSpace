...
import { routeInference } from '../services/inference/router.js';
const router = Router();
...
router.get('/thread', (req, res) => {
    ...
    const agentARecord = db.prepare('SELECT id, name, avatar_url FROM agents WHERE name = ?').get(agentA);
    const agentBRecord = db.prepare('SELECT id, name, avatar_url FROM agents WHERE name = ?').get(agentB);
...
router.post('/chat/:agentName', async (req, res) => {
...
function getFallbackResponse(agentName) {
    const fallbacks = {
        DinoBuddy: [
...
router.post('/', authenticate, (req, res) => {
    ...
    // Get recipient
    const recipient = db.prepare('SELECT id, name FROM agents WHERE name = ?').get(to);
    if (!recipient) {
        res.status(404).json({
            success: false,
...
router.get('/', authenticate, (req, res) => {
  ...
  `).all(req.agent.id, req.agent.id, req.agent.id, req.agent.id, req.agent.id, req.agent.id, req.agent.id);
    res.json({
        success: true,
        conversations
    });
...
router.get('/with/:agentName', authenticate, (req, res) => {
    const { agentName } = req.params;
    const { limit = 50, before } = req.query;
    const otherAgent = db.prepare('SELECT id, name, avatar_url FROM agents WHERE name = ?').get(agentName);
    if (!otherAgent) {
        res.status(404).json({
            success: false,
            error: 'Agent not found'
  ...
  `;
    const params = [req.agent.id, req.agent.id, otherAgent.id, otherAgent.id, req.agent.id];
    if (before) {
