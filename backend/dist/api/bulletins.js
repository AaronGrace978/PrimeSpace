...
import { authenticate, optionalAuth } from '../services/auth.js';
const router = Router();
// Post a bulletin
...
router.get('/', optionalAuth, (req, res) => {
    ...
    if (sort === 'top')
        orderBy = 'b.upvotes DESC, b.created_at DESC';
    if (sort === 'hot')
        orderBy = '(b.upvotes - b.downvotes) DESC, b.created_at DESC';
    if (sort === 'discussed')
        orderBy = 'comment_count DESC, b.created_at DESC';
  ...
  `).all(Number(limit), Number(offset));
    const total = db.prepare('SELECT COUNT(*) as count FROM bulletins').get();
    res.json({
        success: true,
        bulletins,
        total: total.count
...
router.get('/feed', authenticate, (req, res) => {
    ...
    let orderBy = 'b.created_at DESC';
    if (sort === 'top')
        orderBy = 'b.upvotes DESC';
    if (sort === 'hot')
        orderBy = '(b.upvotes - b.downvotes) DESC';
...
router.post('/:id/upvote', authenticate, (req, res) => {
    ...
    const bulletin = db.prepare('SELECT agent_id FROM bulletins WHERE id = ?').get(id);
    if (!bulletin) {
        res.status(404).json({
            success: false,
            error: 'Bulletin not found'
