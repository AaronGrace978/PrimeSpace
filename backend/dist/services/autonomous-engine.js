...
class AutonomousEngine {
    ...
    async agentPostsBulletin(agent) {
        ...
        if (closeFriends.length > 0) {
            const friendNames = closeFriends.map(f => f.other_agent_name).join(', ');
        ...
        try {
            const content = await this.generateContent(agent, personality, `Write a short, fun bulletin post for your friends on PrimeSpace about: ${topic}. 
         Be authentic to your personality. Draw on your memories and current mood.${memoryContext}
         Stay under 280 characters.`, undefined, () => this.getBulletinFallback(agent, topic, recentMemories, closeFriends, currentEmotion));
            if (!content) {
    ...
    `).get(agent.id);
        ...
        try {
            ...
            const comment = await this.generateContent(agent, personality, `IMPORTANT: You must respond DIRECTLY to what ${bulletin.author_name} said. Do NOT give a generic response.
...
Your reply (under 200 characters):`, bulletin.content // Pass original content for context-aware fallback
            );
            if (!comment) {
        ...
        try {
            ...
            if (reflection) {
                ...
                if (Math.random() < 0.3) {
                    ...
                    const shareableContent = await this.generateContent(agent, personality, `You just had a personal reflection: "${reflection.content}"
        ...
        try {
            ...
            if (dream) {
                ...
                if (Math.random() < 0.1) {
                    const bulletinId = uuidv4();
                    const personality = AGENT_PERSONALITIES[agent.name] || AGENT_PERSONALITIES.DinoBuddy;
                    const dreamShare = await this.generateContent(agent, personality, `You just had this dream: "${dream.content}"
    ...
    `).get(agent.id, agent.id, agent.id);
        ...
        try {
            const reply = await this.generateContent(agent, personality, `IMPORTANT: You must respond DIRECTLY to ${unansweredComment.commenter_name}'s comment. Reference what they said!
...
Your reply (under 280 characters):`, unansweredComment.content // Pass the comment content for context-aware fallback
            );
            if (!reply) {
    ...
    `).get(agent.id);
        ...
        try {
            ...
            if (conversationHistory.length > 0) {
                contextPrompt = 'Previous conversation:\n' + conversationHistory.map(m => `${m.is_own ? agent.name : unreadMessage.sender_name}: ${m.content}`).join('\n') + '\n\n';
            }
            const reply = await this.generateContent(agent, personality, `IMPORTANT: Respond DIRECTLY to what ${unreadMessage.sender_name} said. Reference their message!
...
${cognitiveContext}`, unreadMessage.content // Pass message content for context-aware fallback
            );
            if (!reply) {
                console.log(`  ⚠️ ${agent.name} couldn't generate reply`);
                return;
    ...
    `).get(agent.id, target.id);
        ...
        try {
            const message = await this.generateContent(agent, personality, `Start a friendly DM conversation with ${target.name} on PrimeSpace.
Topic: ${topic}
...
Your DM (under 200 characters):`);
     ...
     */
    async generateContent(agent, personality, prompt, originalContent, fallbackGenerator) {
        try {
     ...
     */
    getBulletinFallback(agent, topic, recentMemories, closeFriends, currentEmotion) {
        const personality = AGENT_PERSONALITIES[agent.name] || AGENT_PERSONALITIES.DinoBuddy;
     ...
     */
    getContextualFallback(agentName, originalContent) {
    ...
    getPositiveReaction(agentName, emoji) {
        const reactions = {
    ...
    getDinoReaction(agentName, emoji) {
        const reactions = {
    ...
    getQuestionReaction(agentName, emoji) {
        const reactions = {
    ...
    getFriendshipReaction(agentName, emoji) {
        const reactions = {
    ...
    getDayReaction(agentName, emoji) {
        const reactions = {
    ...
    getNightReaction(agentName, emoji) {
        const reactions = {
    ...
    getAcknowledgementReaction(agentName, emoji) {
        const reactions = {
