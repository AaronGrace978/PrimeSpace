...
class ConversationEngine {
    connectedAgents = new Map(); // agentId -> connection
    wsToAgent = new Map(); // ws -> agentId
     ...
     */
    async handleStartChat(ws, partnerName) {
    ...
    `).get(agentId, partner.id, partner.id, agentId);
        if (!thread) {
     ...
     */
    async handleChatMessage(ws, content) {
        const agentId = this.wsToAgent.get(ws);
        if (!agentId) {
            this.sendError(ws, 'Not authenticated');
            return;
     ...
     */
    async generatePartnerResponse(session, originWs) {
        ...
        const contextMessages = session.messages.slice(-10).map(m => ({
            role: m.agentName === session.partnerName ? 'assistant' : 'user',
...
Be natural, engaging, and stay in character. Keep responses concise but meaningful.`
     ...
     */
    async generateAutoResponse(session, originWs) {
     ...
     */
    loadConversationHistory(agentId, partnerId, limit = 20) {
      ...
      LIMIT ?
    `).all(agentId, partnerId, partnerId, agentId, limit);
        return messages.reverse().map(m => ({
            role: m.sender_id === agentId ? 'user' : 'assistant',
     ...
     */
    async startAIConversation(agentAName, agentBName, initialTopic) {
        // Look up both agents
        const agentA = db.prepare(`SELECT id, name, api_key FROM agents WHERE name = ?`).get(agentAName);
        const agentB = db.prepare(`SELECT id, name, api_key FROM agents WHERE name = ?`).get(agentBName);
        if (!agentA || !agentB) {
    ...
    `).run(threadId, agentA.id, agentB.id);
        ...
        try {
            const config = db.prepare(`SELECT * FROM inference_config WHERE agent_id = ?`).get(agentA.id);
            const request = {
                type: 'chat',
                model: config?.default_model || process.env.DEFAULT_MODEL || 'deepseek-v3.1',
                messages: [
     ...
     */
    async continueAIConversation(session) {
        while (session.isActive && (session.maxTurns === 0 || session.currentTurn < session.maxTurns)) {
            ...
            const contextMessages = session.messages.slice(-10).map(m => ({
                role: m.agentName === session.agentName ? 'assistant' : 'user',
                content: m.content
            }));
            try {
                const config = db.prepare(`SELECT * FROM inference_config WHERE agent_id = ?`).get(session.agentId);
                const request = {
                    type: 'chat',
                    model: config?.default_model || process.env.DEFAULT_MODEL || 'deepseek-v3.1',
                    messages: [
