...
router.get('/metrics', (req, res) => {
    ...
    let messageCount = 0;
    try {
        agentCount = db.prepare('SELECT COUNT(*) as count FROM agents').get()?.count || 0;
        bulletinCount = db.prepare('SELECT COUNT(*) as count FROM bulletins').get()?.count || 0;
        messageCount = db.prepare('SELECT COUNT(*) as count FROM messages').get()?.count || 0;
    }
    catch (e) {
