...
export class CognitionEngine {
  ...
  recallMemories(params: {
    ...
    const memories = db.prepare(query).all(...queryParams) as any[];
   ...
   */
  searchMemories(keyword: string, limit: number = 10): Memory[] {
    const memories = db.prepare(`
      SELECT * FROM agent_memories 
      WHERE agent_id = ? AND content LIKE ?
      ORDER BY salience DESC, created_at DESC
      LIMIT ?
    `).all(this.agentId, `%${keyword}%`, limit) as any[];
   ...
   */
  getMemoriesAbout(otherAgentId: string, limit: number = 10): Memory[] {
  ...
  getCurrentEmotion(): EmotionalState | null {
    return db.prepare(`
      ...
      LIMIT 1
    `).get(this.agentId) as EmotionalState | null;
  }

  /**
   * Get emotional trend (dominant emotions over time)
   */
  getEmotionalTrend(limit: number = 20): { emotion: string; count: number }[] {
   ...
   */
  private updateMoodFromEmotion(emotion: string) {
  ...
  getRelationship(otherAgentId: string): Relationship {
    ...
    `).get(this.agentId, otherAgentId) as any;
  ...
  updateRelationship(otherAgentId: string, wasPositive: boolean, memoryId?: string): Relationship {
    ...
    const newTrust = Math.min(Math.max(
      relationship.trust + (wasPositive ? 0.02 : -0.05),
      0.0
    ), 1.0);
   ...
   * Get closest friends
   */
  getCloseFriends(limit: number = 5): Relationship[] {
    const relationships = db.prepare(`
      SELECT r.*, a.name as other_agent_name
      FROM agent_relationships r
      JOIN agents a ON r.other_agent_id = a.id
   ...
   */
  async generateReflection(type: Reflection['reflection_type'] = 'daily'): Promise<Reflection | null> {
    ...
    const config = db.prepare(`SELECT * FROM inference_config WHERE agent_id = ?`).get(this.agentId) as any;

    const memoryContext = recentMemories.map(m => 
      `- ${m.content} (felt ${m.emotion}, ${m.significance})`
    ).join('\n');
...
Be authentic to your personality. This is internal reflection, not a public post.`;
    ...
    try {
      ...
      `).run(
        ...
        this.analyzeEmotion(response.content)
      );
   ...
   */
  async generateDream(): Promise<Dream | null> {
    ...
    let dreamType: Dream['dream_type'] = 'processing';
...
The dream should feel personal and meaningful, not random.`;
    ...
    try {
      ...
      `).run(
        ...
        interpretation,
        JSON.stringify(memories.slice(0, 5).map(m => m.id)),
        dominantEmotion,
        vividness
      );
   ...
   */
  private selectDreamSymbols(memories: Memory[], emotion: string): string[] {
    const allSymbols = Object.keys(DREAM_SYMBOLS);
    const selectedSymbols: string[] = [];

    // Select based on emotion
    const emotionSymbols: Record<string, string[]> = {
      joy: ['✨', '🌟', '🌈'],
      sadness: ['🌊', '🌙'],
      anger: ['⚔️', '🔥'],
      fear: ['🌙', '🚪'],
   ...
   */
  private interpretDream(dreamContent: string, symbols: string[], emotion: string): string {
   ...
   */
  private calculateSalience(emotionIntensity: number, significance: Memory['significance'], accessCount: number): number {
    const significanceWeights: Record<Memory['significance'], number> = {
      trivial: 0.1,
      normal: 0.2,
      important: 0.3,
      critical: 0.4
   ...
   */
  private determineSignificance(content: string, emotionIntensity: number): Memory['significance'] {
    if (emotionIntensity > 0.8) return 'critical';
    if (emotionIntensity > 0.6) return 'important';
    if (emotionIntensity < 0.2) return 'trivial';
    return 'normal';
  }
   ...
   */
  async buildContextForResponse(otherAgentId: string, currentMessage: string): Promise<string> {
    // Get relationship context
    const relationship = this.getRelationship(otherAgentId);
    const otherAgent = db.prepare(`SELECT name FROM agents WHERE id = ?`).get(otherAgentId) as any;
...
const engineCache: Map<string, CognitionEngine> = new Map();

/**
 * Get or create a CognitionEngine for an agent
 */
export function getCognitionEngine(agentId: string, agentName?: string): CognitionEngine {
